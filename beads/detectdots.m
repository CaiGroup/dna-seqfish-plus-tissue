function [dots, dotsLogical, logImage] = detectdots(image, threshold, varargin)
...
% Date: 1/28/2019
    ...
    if debug == 1
        ...
        imshow(max(image, [], 3), [min(min(max(image,[],3))) mean(mean(max(image, [], 3))) + 2000], 'InitialMagnification', 'fit');
        hold on;
        [v2,v1] = find(max(dotsLogical, [], 3) == 1);
        scatter(v1(:), v2(:), 75);
        hold off;
        if saveImage
            savefig(h, saveFigPath);
        end
    end
