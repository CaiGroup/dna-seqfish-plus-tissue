function [threshold,points,intensity] = getthreshold(image, options, varargin)
...
% Options, try and catch code adapted from 'loadtiff.m' by YoonOh Tak 2012
    ...
    if numvarargs > argsLimit
        error('src:getthreshold:TooManyInputs', ...
    ...
    %% Check options structure for extra data
        ...
        if nargin <= 1
            ...
            options.typedetectdots = 'log'; % 'introns' or 'exons'
        ...
        if options.processimages 
            ...
            image1Process = imagejbackgroundsubtraction(image, ...
                uniqueString, options.expdir, options.backradius, options.sliding);
        ...
        if strcmp(options.threshold, 'choose')
           ...
           %disp('Completed Threshold for Image 2');
            ...
            [points, intensity, ~, ~] = detectdotsv2(thresholdImage, threshold, ...
                options.typedetectdots, options.savefigs, options.pathsavedots);
        end
