...
% low set as new low if not move 20% again from the threshold
    ...
    if abs(numpointError) > errorThreshold
        if numpointError > 0
            high = startThreshold;
            newThreshold = high;
            higherror = numpointError;
            lowRange = false;
        elseif numpointError < 0
            low = startThreshold;
            newThreshold = low;
            lowerror = numpointError;
            lowRange = true;
        end
    else
        ...
        if abs(numpointError) > errorThreshold
            ...
            if lowRange % started with low range
                if numpointError < 0 % change to new low range
                    low = newThreshold;
                    lowerror = numpointError;
                    newThreshold = newThreshold *1.1; % increase by 10%
                else
                    higherror = numpointError;
                    high = newThreshold;
                    break;
                ...
                if numpointError > 0
                    higherror = numpointError;
                    high = newThreshold;
                    newThreshold = newThreshold *0.9; % decrease by 10%
                else
                    lowerror = numpointError;
                    low = newThreshold;
                    break;
