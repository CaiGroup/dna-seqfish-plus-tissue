function [points, threshold] = matchpointsims(image, varargin)
...
% Updated: 8/12/2019
    ...
    optargs = {[], 'exons', true, 3, false, [], sqrt(3), false, pwd, 'gaussian'};
    % assign defaults
    optargs(1:numvarargs) = varargin;
    % Default Value of ref image is 1
    [threshold, typedots, process, backradius, sliding, target, searchradius, debug, saveFigPath, superres] = optargs{:};
    ...
    if process
        ...
        for c = 1:numOfChannels
            I{c} = imagejbackgroundsubtraction(image{c}, ...
                uniqueString, saveFigPath, backradius, sliding);
        end
    else
        I = image;
    end
    ...
    switch superres
        case 'gaussian'
            dotsRef = getgaussian(dots, image{1});
        case 'log'
            [dotsRef, intensityRef] = SuperResPoints(dots,image{1},1,1);
    ...
    for j = 2:numOfChannels
        [dotsMoving{j}, ~, ~, ~] = detectdotsv2(I{j}, threshold(j), typedots);
        %dotsMoving{j} = getgaussian(dotsMoving{j}, image{j});
        [dotsMoving{j}, intensityCh] = SuperResPoints(dotsMoving{j}, image{j},1,1);
        [matchFinal{j}, mismatchFinal{j}, ~, ~] = matchpoints(dotsRef, dotsMoving{j}, searchradius, intensityRef, intensityCh);
    end
