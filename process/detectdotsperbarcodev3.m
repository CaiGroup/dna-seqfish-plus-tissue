...
% Updated: 8/4/2019
    ...
    [detectTypePoints, refinePointLocation, pointsSavePath, multiplier] = optargs{:};
    ...
    for barcode = barcodeArray
        ...
        for ch = channelArray
            ...
            switch refinePointLocation
                ...
                case 'radial'
                    points{barcode}(ch).channels = getradialcenter(points{barcode}(ch).channels, imageStruct{hybIndex});
                case 'radial3d'
                    [points{barcode}(ch).channels, ~] = SuperResPoints(points{barcode}(ch).channels,imageStruct{hybIndex},1,1);
                otherwise
