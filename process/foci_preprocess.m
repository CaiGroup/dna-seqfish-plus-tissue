function [nucleolus]= foci_preprocess(PathName, Im, channel, regchan, regist)

fld = pwd;
Miji;
cd(fld);

path_to_fish = ['path=[' PathName '\HybCycle_' num2str(imnum) '.tif]'];
...
MIJ.run('Save', ['save=[' PathName '\ITS1binary.tif' ']']);
...
save([PathName '\ITS1pixels.mat'],'nucleolus');
