function [points, threshold] = detectdotsperhybcycle(experimentDir, imageStruct, folderArray, channel, varargin)
...
% Email: nicogpt@caltech.edu
    ...
    if numvarargs > 1
        ...
        elseif ~strcmp(varargin{2}, 'none') && ~strcmp(varargin{2}, 'gaussian') && ~strcmp(varargin{2}, 'radial') && ~strcmp(varargin{2}, 'compressed') && ~strcmp(varargin{2}, 'radial3d')
    ...
    for f = 1:numHybs % starts at 1
        fprintf('Finding Dots in HybCycle: %.0f channel', folderArray(f));
        points{f} = struct('channels', cell(numChannels, 1), 'intensity', cell(numChannels, 1), 'scaledIntensity', cell(numChannels, 1));
        for ch = channel
            fprintf('%.0f ', ch);
            ...
            [points{f}(ch).channels, points{f}(ch).intensity, ~, ~] = detectdotsv2(imageStruct{f, ch}, ...
            ...
            switch refinePointLocation
                case 'none'
                    % do nothing
                case 'gaussian'
                    points{f}(ch).channels = getgaussian(points{f}(ch).channels, imageStruct{f, ch});
                case 'radial'
                    points{f}(ch).channels = getradialcenter(points{f}(ch).channels, imageStruct{f, ch});
                case 'compressed'
                    % need to add this part
                case 'radial3d'
                    [points{f}(ch).channels, ~] = SuperResPoints(points{f}(ch).channels,imageStruct{f, ch},1,1);
                    indices = find(points{f,1}(ch).channels(:,1)==0);%find y=0, which were dropped by SuperResPoints.
                    points{f,1}(ch).channels(indices,:) = [];
                    points{f,1}(ch).intensity(indices,:) = [];
                otherwise
