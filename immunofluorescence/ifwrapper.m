function ifwrapper(experimentDir, posArray, hybImageName, cycleArray, ...
...
% experimentDir/processedimages/pos[0-9]/imStartName
    ...
    for position = posArray     
        ...
        imStartName = [hybImageName num2str(position)];
        saveDir = fullfile(experimentDir, 'processedimages', ['pos' num2str(position)]);
        imPath = getfile(saveDir, imStartName, 'match');
        load(imPath, 'hybIms');
        sizeZ = size(hybIms{1,1}, 3);
        % get only the channels used in the if analysis
        if ~isempty(cycleArray)
            I = hybIms(cycleArray+1,chArray);
        else
            I = hybIms(:,chArray);
        end
